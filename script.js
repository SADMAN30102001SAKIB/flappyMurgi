...
// Frame rate independent variables
let lastTime = 0;
const targetFPS = prompt("Enter target FPS (default is 75):", "75") || 75;
const frameTime = 1000 / targetFPS;

let scoresnd = new Audio(
  "https://dl.dropbox.com/s/vupnrhb1sb4q0h5/mario-coin-sound-effect.mp3?dl=0",
);
var audio = new Audio(
  "https://www.dropbox.com/s/qxukfvbbg25qkdd/solo%20.mp3?dl=1",
);
...
window.onload = () => {
  ...
  image.src =
    "https://dl.dropbox.com/s/tqd19quzwr6az54/flappy_bird_copy_188x144.png?dl=0";

  image2.src =
    "https://dl.dropbox.com/s/vg27bpwfddv0g7g/background%20-%20flappy%20bird.png?dl=0";
...
function play(currentTime = 0) {
  ...
  const deltaTime = Math.min((currentTime - lastTime) / frameTime, 2); // Cap delta time to prevent huge jumps
...
class Particle {
  ...
  update(deltaTime = 1) {
    this.x -= (gamespeed + 0.5) * deltaTime;
    this.y += this.v * 2 * deltaTime;
    if (this.r > 1) {
      this.r -= 0.3 * deltaTime;
    }
  }
}

function handleParticles(deltaTime = 1) {
  if (!birdcollided) {
    if (scores % 5 == 0) {
...
function handlePipes(deltaTime = 1) {
